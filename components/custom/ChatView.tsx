...
import { liveAIExplanationState } from "@nextCode/context/live_chat";
...
export default function ChatView() {
  ...
  const { explanation } = liveAIExplanationState();
  ...
  return (
    <>
        ...
        >
            ...
            >
              ...
              <div className="flex-grow rounded-lg">
                <ReactMarkdown
                  ...
                  components={{
                    p: ({ children }) => (
                      <div className="space-y-4 rounded-lg">{children}</div>
                    ),
                    pre: ({ children }) => (
                      <pre className="bg-gray-800 p-4 rounded-lg my-2 overflow-x-auto w-full">
                        {children}
                      </pre>
                    ),
                    code: ({ children }) => (
                      <code className="font-mono bg-gray-800 rounded-lg px-1">
                        {children}
                      </code>
                    ),
                    ul: ({ children }) => (
                      <ul className="list-disc ml-6 my-2 rounded-lg space-y-2">
                        {children}
                      </ul>
                    ),
                    ol: ({ children }) => (
                      <ol className="list-decimal ml-6 my-2 rounded-lg space-y-2">
                        {children}
                      </ol>
                    ),
          ...
          {explanation && (
            <div className="p-3 rounded-lg bg-gray-700 mr-5 mb-2 flex gap-2 items-start">
              <Image
                src="/your-ai-icon.png" // Or any AI icon
                alt="ai"
                width={35}
                height={35}
                className="rounded-full flex-shrink-0"
              />
              <div className="flex-grow rounded-lg whitespace-pre-wrap">
                {explanation}
              </div>
            </div>
          )}
      ...
      <div
        ...
        className="absolute top-6 left-6 cursor-pointer z-40 w-fit"
      >
        <svg
          width="24"
          height="24"
